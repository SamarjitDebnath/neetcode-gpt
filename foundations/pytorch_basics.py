...
class Solution:
    def reshape(self, to_reshape: TensorType[float]) -> TensorType[float]:
        ...
        return torch.round(torch.reshape(to_reshape, shape=[(m*n)//2, 2]), decimals=4)
    ...
    def get_loss(self, prediction: TensorType[float], target: TensorType[float]) -> TensorType[float]:
        ...
        return torch.round(torch.nn.functional.mse_loss(input=prediction, target=target), decimals=4)
